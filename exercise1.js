...
// First, we'll need our HTML "cooking ingredients". Fetch the elements that we will be modifying or manipulating in this exercise. 
let btn = document.getElementById("the-button");
let totalClicks = document.getElementById("your-total-clicks")

// Take a close look at the HTML file and think about what user actions are occurring on what element, and what elements are changing in response to those user actions. 
...
// Next, once we have our HTML elements selected, or imported, into our JS file, we can attach an event listener to them. Attach a "click" event listener to the button that, when triggered, will run a callback function that increases the total clicks element by 1 with each click. 
    ...
    btn.addEventListener("click", () => {
            totalClicks.innerText = Number(totalClicks.innerText) + 1
